...
function App() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-bold text-orange-600">CARGANDO DESARROLLO HUMANO...</div>;

  if (!user) {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-sm text-center">
          <img src="https://desarrollohumano.chubut.gov.ar/wp-content/uploads/2024/04/logochubut.png" className="w-32 mx-auto mb-6" alt="Logo Chubut" />
          ...
          <form onSubmit={(e) => { e.preventDefault(); loginWithEmailPassword(email, password); }} className="space-y-4">
            ...
            <button className="w-full bg-orange-600 text-white p-3 rounded-xl font-bold hover:bg-orange-700 transition">Entrar al Sistema</button>
          </form>
      ...
      <div className="ml-64 p-10 w-full">
        ...
        {activeMenu === 'inventario' && (
          <div className="bg-white rounded-3xl p-8 shadow-sm">
            <div className="flex gap-4 mb-8">
              ...
              <button onClick={() => setShowForm(!showForm)} className="bg-orange-600 text-white px-6 py-3 rounded-xl font-bold hover:shadow-lg transition">
            ...
            {showForm && (
              <form onSubmit={handleSubmit} className="grid grid-cols-3 gap-4 mb-8 p-6 bg-slate-50 rounded-3xl border border-slate-200">
                <input type="text" placeholder="Nombre" value={formData.nombre} className="p-3 border rounded-xl bg-white" onChange={e => setFormData({...formData, nombre: e.target.value})} required />
        ...
        {activeMenu === 'reportes' && (
          <div className="bg-white p-12 rounded-3xl text-center shadow-sm">
            <h3 className="text-xl font-black mb-6 text-slate-800">Generador de Planillas</h3>
            <div className="flex justify-center gap-6">
              <button onClick={() => {
                const ws = XLSX.utils.json_to_sheet(inventoryItems);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Desarrollo Humano Stock");
                XLSX.writeFile(wb, `Inventario_DesarrolloHumano_${inventoryType}.xlsx`);
              }} className="bg-green-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-green-700 transition shadow-lg">Exportar a Excel</button>
